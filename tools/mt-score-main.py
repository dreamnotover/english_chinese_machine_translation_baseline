...
def check(args):
    ...
    if not os.path.exists(args.ref_sgm):
        check_pass = False
        print("ERROR: not find %s"%args.ref_sgm, file=sys.stderr)
    if not os.path.exists(args.src_sgm):
        check_pass = False
        print("ERROR: not find %s"%args.src_sgm, file=sys.stderr)
    if not os.path.exists(args.hyp_sgm):
        check_pass = False
        print("ERROR: not find %s"%args.hyp_sgm, file=sys.stderr)
    return check_pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        if gps:
            ...
            print("ERROR: unable to get bleu and nist score", file=sys.stderr)
            sys.exit(1)
    except:
        print("ERROR: exception during calculating bleu score", file=sys.stderr)
